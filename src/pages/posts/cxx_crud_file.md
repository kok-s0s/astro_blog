...
<details><summary>所有测试代码，记住，测试代码应当与你功能代码的「 体量 」差不多大</summary>
...
dat 是对二进制的操作，我这里用的是 C 语言的 `File` 来处理，为什么呢？

因为原先的 `FileTools` 类中，前辈们都用的它。

有句话是这样的：如果代码能够正常跑起来，且多年未曾出现错误，那就还是对这些保持谨慎的态度。
...
呃，也用的[开源库](https://github.com/kok-s0s/cxx_crud_file/blob/main/bmp/BMP.h)，是公司的前辈 clone 的，我没找到来源，不过还是日常感谢无私的开源共享者。
...
这个开源库的接口考虑的很全面了，处理目前的业务够用了，就没做二次封装，不包了，直接用即可。
...
原先前辈在 Github 找的，软件组的大家实际使用中，根据业务的需求，有做一些修改，加了个 `wchar` 的处理。
